...
function generateQuery(str){
    ...
    const filteredArray = words.filter(function(x) { 
        return stopwords.indexOf(x) < 0;
      });
    filteredArray.forEach(function (item,index, array){
        if (index === (array.length -1)) {
            // This is the last one.
            query_string_before +="W='"+item+"'"
        }else{
            query_string_before +="W='"+item+"',"
        }
    });
    
    return(query_string_before+query_string_after+"")
}
...
function handleProject(app, cb) {
    ...
    async.series([
        //caching serviceinfo
        next=>{
            data = common.updateProjectMag(query,app.id)
            // console.log(data)
            next()
        },
        //now save the app
